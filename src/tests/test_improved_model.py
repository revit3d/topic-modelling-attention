...
def test_step(model, phi, n_t, data, doc_bounds, config):
    ...
    phi_it_model, phi_model, theta_model, n_t_model = model._step(
        batch=data, ctx_bounds=doc_bounds, phi=phi.T, n_t=n_t, grad_reg=grad_reg
    )
    ...
    assert_allclose(theta_model, theta_primitive.T, rtol=1e-5, atol=1e-6)
    assert_allclose(n_t_model, n_t_primitive, rtol=1e-5, atol=1e-6)
    assert_allclose(phi_model, phi_primitive.T, rtol=1e-5, atol=1e-6)
